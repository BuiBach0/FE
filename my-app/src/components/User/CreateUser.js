...
function CreateUser() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const response = await axiosInstance.post("/users/create-user", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    <SiderLayout  >
      ...
      <Content>
        <Row style={{ height: "calc(100vh - 64px)" }}>
          ...
          <Col span={18} style={{ backgroundColor: "#ECF0F1", padding: "20px" }}>
            <Content className="content">
              <div className="detail-section"  style={{ backgroundColor: "#F0F4F8" }}>
